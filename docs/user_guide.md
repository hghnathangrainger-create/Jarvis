...
### Jarvis Intelligence: Tool Selection (Phase 90, Batches 2/3; Phase 91, Batch 1)

| Command | Tier | Does |
|---|---|---|
| `ask jarvis to: <request>` | GREEN capabilities: no approval. YELLOW capability (focus update): requires your explicit approval. | Asks Jarvis to select at most one explicitly allowlisted capability - showing your manually-maintained project state, reporting basic system health, listing your configured schedules, or listing your most recently stored memories (all GREEN, no approval), or updating only the project state's `focus` field (YELLOW, approval-gated, verified after) - and, if selected and safe, actually runs it. |

`ask jarvis to:` builds on the exact same bounded memory/project-state context assembly as `ask jarvis:` above, then asks the AI to choose between six outcomes: **show** the current project state (`project_state_show`, GREEN, no approval), **update** only its focus field (`project_state_update_focus`, YELLOW, requires your approval), **report basic system health** (`health_check`, GREEN, no approval), **list your configured schedules** (`schedule_list`, GREEN, no approval), **list your most recently stored memories** (`memory_list_recent`, GREEN, no approval), or **honestly report that no supported capability applies** — Jarvis never forces a match onto an unrelated request, and never invents another capability. Every AI response is strictly, deterministically validated (exact JSON shape, known capability, matching arguments) before anything else happens; a malformed or unrecognised response is rejected outright, never guessed at or repaired. Each of the three Phase 91 read-only capabilities takes **exactly zero arguments** from the AI — where the real tool underneath needs an internal, non-user-facing selector (for example, `memory_list_recent`'s real tool needs to be told which of its several operations to run), that selector is a fixed literal chosen by the capability's own catalog entry, never supplied or influenced by the model.
