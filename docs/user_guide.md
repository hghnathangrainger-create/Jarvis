...
| `jarvis brain status`, `show jarvis brain` | Reports Jarvis's own current AI/reasoning configuration, real memory/approval/workflow counts, tool registry size, and an honest "current limits" section (Phase 86, Batch 1). Read-only; never shows the API key's value; never calls the Claude API, any other AI provider, a subprocess, or the web; has no live knowledge of the current git branch, commit, or test suite result. |
...
**Verified Action Context (Phase 100, Batch 2)** adds a third, optional piece of context to both `ask jarvis:` and `ask jarvis to:` (below): up to five short, deterministic lines of bounded historical evidence about your two "and then" workflows (updating your project phase, and enabling a schedule) — for example, that schedule 12's enable was verified, that an update to your project phase is still awaiting your approval, or that enabling schedule 7 was interrupted and its completion isn't confirmed. This is **historical evidence, not current state** — every line carries its own timestamp and says what was verified *then*, never what is true *now* — and it never bypasses a fresh read, an approval, or a verification check for your current request. It covers only the two "and then" workflows above; ordinary single-step actions, and any workflow Jarvis has no dedicated durable record for, are never included. It reads only durable records already created for other reasons — no new memory is created, no new table exists, and nothing here is ever generated by an AI model; if this evidence can't be read for any reason, your request proceeds exactly as if it were never available. This is not a "repeat my last action" or "resolve that schedule" feature — Jarvis still needs you to name the schedule id or phase value in your own current request every time.

### Jarvis Intelligence: Tool Selection (Phase 90, Batches 2/3; Phase 91, Batches 1/2; Phase 93, Batch 1; Phase 94, Batch 2; Phase 95; Phase 96; Phase 98, Batch 3; Phase 99, Batch 1; Phase 100, Batch 2)

| Command | Tier | Does |
|---|---|---|
| `ask jarvis to: <request>` | GREEN capabilities: no approval. YELLOW capabilities (focus update, phase update, schedule enable, schedule disable): require your explicit approval. | Asks Jarvis to select at most one explicitly allowlisted capability - showing your manually-maintained project state, reporting basic system health, listing your configured schedules, listing your most recently stored memories, searching your stored memories for a query, showing your recent approval history, showing your recent workflow history, or showing whether one of your existing schedules, by its exact id, is currently enabled or disabled (all GREEN, no approval), or updating only the project state's `focus` or `phase` field, or enabling or disabling one of your existing schedules by its exact id (all four YELLOW, approval-gated, verified after) - and, if selected and safe, actually runs it. |

`ask jarvis to:` builds on the exact same bounded memory/project-state/Verified-Action-Context assembly as `ask jarvis:` above, then asks the AI to choose between thirteen outcomes: **show** the current project state (`project_state_show`, GREEN, no approval), **update** only its focus field (`project_state_update_focus`, YELLOW, requires your approval), **update** only its phase field (`project_state_update_phase`, YELLOW, requires your approval — Phase 96), **report basic system health** (`health_check`, GREEN, no approval), **list your configured schedules** (`schedule_list`, GREEN, no approval), **enable one of your existing schedules by its exact id** (`schedule_enable`, YELLOW, requires your approval — Phase 94, Batch 2), **disable one of your existing schedules by its exact id** (`schedule_disable`, YELLOW, requires your approval — Phase 95), **list your most recently stored memories** (`memory_list_recent`, GREEN, no approval), **search your stored memories for a query** (`memory_search`, GREEN, no approval), **show your recent approval history** (`approval_history`, GREEN, no approval), **show your recent workflow history** (`workflow_history`, GREEN, no approval), **show whether one of your existing schedules, by its exact id, is currently enabled or disabled** (`schedule_show_enabled_state`, GREEN, no approval — Phase 99, Batch 1), or **honestly report that no supported capability applies** — Jarvis never forces a match onto an unrelated request, and never invents another capability. Every AI response is strictly, deterministically validated (exact JSON shape, known capability, matching arguments) before anything else happens; a malformed or unrecognised response is rejected outright, never guessed at or repaired. Six of the twelve capabilities (all but `project_state_update_focus`, `project_state_update_phase`, `memory_search`, `schedule_enable`, `schedule_disable`, and `schedule_show_enabled_state`) take **exactly zero arguments** from the AI; `memory_search` (Phase 91 Batch 2) takes **exactly one required argument**, `value` (a non-empty string, at most 500 characters, the same generic string-hygiene rule already applied to `project_state_update_focus`'s own argument); `project_state_update_phase` (Phase 96) takes the identical single required `value` argument, with identical string-hygiene rules; `schedule_enable`/`schedule_disable`/`schedule_show_enabled_state` (Phase 94, Batch 2; Phase 95; Phase 99, Batch 1) each take **exactly one required argument**, `schedule_id` (a real integer — not a string, not a boolean, not a decimal — attributed only from an unambiguous "schedule &lt;number&gt;" mention in your own request, never guessed or bounded to an invented range). Where the real tool underneath needs an internal, non-user-facing selector (for example, `project_state_update_focus`/`project_state_update_phase` each need the real `project_state_update` tool to be told which field to write; `memory_list_recent`/`memory_search`'s real tool needs to be told which of its several operations to run; and `approval_history`/`workflow_history`'s real tools need to be told to run their own default "history" listing), that selector is a fixed literal chosen by the capability's own catalog entry, never supplied or influenced by the model - and `memory_search`'s validated `value` is mapped internally to the real tool's own `query` field, also never something the model names or controls. `approval_history`/`workflow_history` (Phase 93, Batch 1) each return the same real tool's existing, hard-bounded default listing (at most 20 most-recent entries) already shown by the deterministic `show approval history`/`show workflow history` commands - never a different, AI-specific view, larger limit, or additional field. There is no AI-selectable way to update branch, commit, or suite result, create a schedule, or target a schedule by name — those remain deterministic-only commands (`update jarvis project state: branch=...`/`commit=...`/`suite=...`, `schedule web search summary for ...`).

**Showing a schedule's enabled state** (`schedule_show_enabled_state`, Phase 99, Batch 1) is a real, independent, GREEN, read-only capability - unlike `schedule_list`, whose fixed 50-schedule, oldest-first output cannot guarantee an arbitrary schedule's presence once you have more than 50 configured, this capability targets exactly one schedule by its exact id and either finds it or honestly reports it does not exist - never fabricating "disabled" for a schedule that was never found. It has no deterministic typed-command grammar of its own; it is reachable only through `ask jarvis to:`.

**Showing project state** goes through a real security check requiring GREEN; if that ever fails unexpectedly, Jarvis refuses honestly. A real, successful result is always the actual tool's own output, labelled `[Jarvis tool result]` — never an AI paraphrase of it.

**Updating the focus or phase field** (Phase 90, Batch 3; Phase 96) each follow the identical two-step, approval-gated workflow, targeting the same real `project_state_update` tool with a different fixed field literal: (1) Jarvis pauses and asks you to approve the exact update, exactly like any other YELLOW action; (2) once approved, the real update runs, and a second, internal, read-only step immediately reads the stored value back and checks it **exactly matches** what you approved — never a "looks close enough" comparison, never an AI judgement call. Both capabilities reuse the identical internal read-only verifier; only which field it reads back differs, and that field is fixed by Jarvis itself, never something the AI supplies or overrides. The final response always says plainly whether verification **succeeded**, **found a different value than requested**, or **could not be completed** — a tool reporting success is never, by itself, presented as a confirmed update. Declining, or the update itself failing, means nothing is changed and no verification is attempted. This whole exchange survives a restart: if Jarvis is stopped after asking for your approval, the pending request and the exact value you'd be approving are both safely stored, and approving it later still works. Zero retries and zero replans apply throughout — a failure or mismatch is always reported honestly, never silently retried. **Phase (like every ProjectState field) is manually recorded only** — updating it through `ask jarvis to:` still requires you to tell Jarvis the new value; nothing is ever auto-detected from Git, a phase document, a commit, a test result, or the filesystem.

**Updating the phase, then showing the result** (Phase 98, Batch 3) is the first fixed, narrow exception to the single-capability rule above: `ask jarvis to: update my project phase to <value> and then show my project state` — joined by the exact words "and then" — first updates the phase field exactly like the phase-update capability above (approval required, then verified as above), and, **only if that verification succeeds**, also shows the resulting project state in the very same reply, without a second request. This is one of only two two-step combinations Jarvis ever recognizes this way; no other pair of capabilities, no reversed order, and no chain of more than two steps is ever supported, today or as a general mechanism. A decline, an approval timeout, or a verification failure or mismatch ends the request with an honest explanation and never shows the project state. This exchange survives a restart exactly like every other approval-gated workflow: if Jarvis is stopped after asking for your approval, or after the update but before the read-back completes, resuming later still reaches an honest, correct outcome — never a silent skip, a duplicate update, or a fabricated result.

**Enabling a schedule, then confirming its state** (Phase 99, Batch 3) is the second, and only other, fixed exception: `ask jarvis to: enable schedule <id> and then check the enabled state of schedule <same id>` — joined by the exact words "and then", naming the identical schedule id in both clauses — first enables the exact schedule exactly like the enable capability below (approval required, then verified as above), and, **only if that verification succeeds**, also shows that exact schedule's resulting enabled state in the very same reply. The schedule id must match exactly between the two clauses — a differing id is never accepted, and Jarvis never infers or substitutes one id for the other. `and then disable`, `and then list`, a reversed order, or any other pairing is never recognized this way — only `enable` paired with the exact-state check, in that order. A decline, an approval timeout, or a verification failure or mismatch ends the request with an honest explanation and never shows the schedule's state. This exchange survives a restart exactly like the phase-update-then-show workflow above.

**Enabling or disabling a schedule** (Phase 94, Batch 2; Phase 95) each follow the exact same two-step, approval-gated shape, targeting the real `schedule_enable`/`schedule_disable` tool instead: (1) Jarvis pauses and asks you to approve enabling or disabling the exact schedule id you named; (2) once approved, the real schedule is updated, and a second, internal, read-only step immediately re-reads that same schedule and checks its stored `enabled` state is **exactly `True`** (for enable) or **exactly `False`** (for disable) — never assumed just because the write reported success. Both capabilities reuse the identical internal read-only check; only the value each expects differs, and that expected value is fixed by Jarvis itself, never something the AI supplies or overrides. The final response always says plainly whether verification **succeeded**, **found the schedule in the opposite state**, or **could not be completed** (for example, if the schedule no longer exists by the time verification runs). Declining, or the enable/disable itself failing (for example, naming a schedule id that doesn't exist), means nothing is changed and no verification is attempted — and a mismatch or a verifier failure never triggers an automatic retry. Schedule ids always come from `list schedules`/`show schedules` output — Jarvis never invents one, and an ambiguous or multiple-candidate id in your request is refused rather than guessed. Both exchanges survive a restart exactly like the focus/phase-update workflows, and there is no AI-selectable way to create, rename, or search for a schedule — only enable and disable, each by exact id.
...
- `ask jarvis to: update my project phase to <your exact value>`
- `ask jarvis to: check jarvis's health`
- `ask jarvis to: show my schedules` (or `list my schedules`)
- `ask jarvis to: enable schedule <your exact id>`
- `ask jarvis to: disable schedule <your exact id>`
...
- `ask jarvis to: show approval history` (or `list approvals history`)
- `ask jarvis to: show workflow history` (or `list workflows history`)
- `ask jarvis to: update my project phase to <your exact value> and then show my project state`
- `ask jarvis to: enable schedule <your exact id> and then check the enabled state of schedule <the same exact id>`
...
**Missing/invalid required-argument guidance (Phase 101).** For five specific actions — enabling a schedule, disabling a schedule, checking a schedule's enabled state, searching your memories, and updating your project phase — if your request clearly identifies the action but leaves out (or malforms) the one value it needs, Jarvis tells you exactly what's missing and gives you the exact command to resend, instead of a generic refusal. For example, `ask jarvis to: enable schedule` gets: *"I need the schedule ID before I can prepare this action. Try: ask jarvis to: enable schedule &lt;schedule id&gt;. Enabling a schedule will still require approval."* This guidance is fully deterministic — Jarvis never guesses the missing value, never fills it in from something you asked earlier, and never fills it in from your project state. You must resend the complete request yourself; a later bare reply like `12` is never treated as answering the question — Jarvis has no memory of the incomplete request to bind it to. This same guidance only ever appears when your *current* request, on its own, clearly and uniquely identifies one of the five supported actions — a vague request, a negated request ("do not enable schedule"), a request matching more than one action, or a combined "and then" request never receives this guidance; those keep their existing, generic refusal message. Risky actions (enabling/disabling a schedule, updating your project phase) still say plainly that approval will be required once you resend a complete request — guidance is never itself an approval, and nothing is ever queued or remembered on your behalf.
...
| **Memories** | A read-only caption disclosing that memories cannot be created, edited, deleted, re-categorized, or AI-summarized from here; a **Category Breakdown** panel (Phase 63) showing a real, current count for every known category — `general`, `personal`, `project`, `preference`, `note` — including an honest zero for a category with no memories, always in that fixed order (never sorted by count, so no category is implied to be more important than another); the existing category filter; recent memories, filterable by category; and, on selecting a row, its id, category, creation time, and full content. |
| **Approval History** | A **Status Breakdown** panel (Phase 64) showing a real, all-time count for every known status — `pending`, `approved`, `declined`, `expired` — including an honest zero, always in that fixed order (never sorted by count); the existing table of past approval requests and decisions — durable history, not a live "awaiting your decision" list; and, on selecting a row, a detail pane showing its request id, action, tier, status, reason, and (once decided) decision reason. |
| **Workflow History** | A **Status Breakdown** panel (Phase 64) tallying the most recently active workflows' current status across all seven known lifecycle names, including honest zeros, always in fixed order — explicitly labeled as scoped to recent activity, never an all-time total; the existing workflow table; and, on selection, that workflow's full recorded transition history, now also showing the correlated Approval Request ID for a `workflow_step_waiting` row (or "—" when none applies). Durable history only, not resumable state. |
