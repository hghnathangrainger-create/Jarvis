...
### Jarvis Intelligence: Tool Selection (Phase 90, Batches 2/3; Phase 91, Batches 1/2; Phase 93, Batch 1; Phase 94, Batch 2)

| Command | Tier | Does |
|---|---|---|
| `ask jarvis to: <request>` | GREEN capabilities: no approval. YELLOW capabilities (focus update, schedule enable): require your explicit approval. | Asks Jarvis to select at most one explicitly allowlisted capability - showing your manually-maintained project state, reporting basic system health, listing your configured schedules, listing your most recently stored memories, searching your stored memories for a query, showing your recent approval history, or showing your recent workflow history (all GREEN, no approval), or updating only the project state's `focus` field, or enabling one of your existing schedules by its exact id (both YELLOW, approval-gated, verified after) - and, if selected and safe, actually runs it. |

`ask jarvis to:` builds on the exact same bounded memory/project-state context assembly as `ask jarvis:` above, then asks the AI to choose between ten outcomes: **show** the current project state (`project_state_show`, GREEN, no approval), **update** only its focus field (`project_state_update_focus`, YELLOW, requires your approval), **report basic system health** (`health_check`, GREEN, no approval), **list your configured schedules** (`schedule_list`, GREEN, no approval), **enable one of your existing schedules by its exact id** (`schedule_enable`, YELLOW, requires your approval — Phase 94, Batch 2), **list your most recently stored memories** (`memory_list_recent`, GREEN, no approval), **search your stored memories for a query** (`memory_search`, GREEN, no approval), **show your recent approval history** (`approval_history`, GREEN, no approval), **show your recent workflow history** (`workflow_history`, GREEN, no approval), or **honestly report that no supported capability applies** — Jarvis never forces a match onto an unrelated request, and never invents another capability. Every AI response is strictly, deterministically validated (exact JSON shape, known capability, matching arguments) before anything else happens; a malformed or unrecognised response is rejected outright, never guessed at or repaired. Seven of the nine capabilities (all but `project_state_update_focus`, `memory_search`, and `schedule_enable`) take **exactly zero arguments** from the AI; `memory_search` (Phase 91 Batch 2) takes **exactly one required argument**, `value` (a non-empty string, at most 500 characters, the same generic string-hygiene rule already applied to `project_state_update_focus`'s own argument); `schedule_enable` (Phase 94, Batch 2) takes **exactly one required argument**, `schedule_id` (a real integer — not a string, not a boolean, not a decimal — attributed only from an unambiguous "schedule &lt;number&gt;" mention in your own request, never guessed or bounded to an invented range). Where the real tool underneath needs an internal, non-user-facing selector (for example, `memory_list_recent`/`memory_search`'s real tool needs to be told which of its several operations to run, and `approval_history`/`workflow_history`'s real tools need to be told to run their own default "history" listing), that selector is a fixed literal chosen by the capability's own catalog entry, never supplied or influenced by the model - and `memory_search`'s validated `value` is mapped internally to the real tool's own `query` field, also never something the model names or controls. `approval_history`/`workflow_history` (Phase 93, Batch 1) each return the same real tool's existing, hard-bounded default listing (at most 20 most-recent entries) already shown by the deterministic `show approval history`/`show workflow history` commands - never a different, AI-specific view, larger limit, or additional field. There is no AI-selectable way to disable a schedule, create a schedule, or target one by name — those remain deterministic-only commands (`disable schedule <id>`, `schedule web search summary for ...`).
...
**Enabling a schedule** (Phase 94, Batch 2) follows the exact same two-step, approval-gated shape as updating the focus field, targeting the real `schedule_enable` tool instead: (1) Jarvis pauses and asks you to approve enabling the exact schedule id you named; (2) once approved, the real schedule is re-activated, and a second, internal, read-only step immediately re-reads that same schedule and checks its stored `enabled` state is **exactly `True`** — never assumed just because the write reported success. The final response always says plainly whether verification **succeeded**, **found the schedule still disabled**, or **could not be completed** (for example, if the schedule no longer exists by the time verification runs). Declining, or the enable itself failing (for example, naming a schedule id that doesn't exist), means nothing is changed and no verification is attempted. Schedule ids always come from `list schedules`/`show schedules` output — Jarvis never invents one, and an ambiguous or multiple-candidate id in your request is refused rather than guessed. This exchange survives a restart exactly like the focus-update workflow, and there is no AI-selectable way to disable, create, rename, or search for a schedule — only enable, by exact id.
