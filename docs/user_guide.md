...
### Jarvis Intelligence: Tool Selection (Phase 90, Batches 2/3; Phase 91, Batches 1/2)

| Command | Tier | Does |
|---|---|---|
| `ask jarvis to: <request>` | GREEN capabilities: no approval. YELLOW capability (focus update): requires your explicit approval. | Asks Jarvis to select at most one explicitly allowlisted capability - showing your manually-maintained project state, reporting basic system health, listing your configured schedules, listing your most recently stored memories, or searching your stored memories for a query (all GREEN, no approval), or updating only the project state's `focus` field (YELLOW, approval-gated, verified after) - and, if selected and safe, actually runs it. |

`ask jarvis to:` builds on the exact same bounded memory/project-state context assembly as `ask jarvis:` above, then asks the AI to choose between seven outcomes: **show** the current project state (`project_state_show`, GREEN, no approval), **update** only its focus field (`project_state_update_focus`, YELLOW, requires your approval), **report basic system health** (`health_check`, GREEN, no approval), **list your configured schedules** (`schedule_list`, GREEN, no approval), **list your most recently stored memories** (`memory_list_recent`, GREEN, no approval), **search your stored memories for a query** (`memory_search`, GREEN, no approval), or **honestly report that no supported capability applies** — Jarvis never forces a match onto an unrelated request, and never invents another capability. Every AI response is strictly, deterministically validated (exact JSON shape, known capability, matching arguments) before anything else happens; a malformed or unrecognised response is rejected outright, never guessed at or repaired. The three Phase 91 Batch 1 read-only capabilities take **exactly zero arguments** from the AI; `memory_search` (Phase 91 Batch 2) takes **exactly one required argument**, `value` (a non-empty string, at most 500 characters, the same generic string-hygiene rule already applied to `project_state_update_focus`'s own argument). Where the real tool underneath needs an internal, non-user-facing selector (for example, `memory_list_recent`/`memory_search`'s real tool needs to be told which of its several operations to run), that selector is a fixed literal chosen by the capability's own catalog entry, never supplied or influenced by the model - and `memory_search`'s validated `value` is mapped internally to the real tool's own `query` field, also never something the model names or controls.
