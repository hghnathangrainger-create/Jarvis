...
- A file delete tool exists (Phase 35, §6) as a quarantine-only move into `.jarvis_trash/`, you can list what's in quarantine including original path when known (Phase 36/37, §6), and every newly quarantined file now has its original path durably recorded (Phase 37) — closing the specific gap that previously blocked a restore command from being designed safely. Not yet built, each a distinct future decision: a `restore file` command itself (Phase 37 only lays the groundwork; it does not restore anything), an `empty trash` command, an automatic cleanup/retention policy, and any dashboard visibility into quarantine contents.
