...
**Phase title:** Voice Interface Planning + Voice Output Foundation
...
**Status:** planning complete; Nathan has recorded temporary selected defaults for several of the decisions below (see the section immediately following this header) — these are explicitly changeable, before or during implementation, and do not close off any option. **Batch 1 (TTS provider abstraction, fake provider, voice output service — foundation only, no CLI wiring, no microphone, no real TTS dependency) is now complete** — see Section 12 for exactly what was delivered and what remains for later batches.
...
To be answered and implemented in a future batch, not now. For reference, `config/settings.py` today is a frozen `Settings` dataclass populated once at startup from environment variables (via `.env`), with typed helpers (`_get_required`, `_get_optional`, `_get_int`, `_get_bool`) and full validation at load time; `ConfigTool` reports a redacted view of the loaded settings (e.g. the API key is reported only as "set"/"not set", never its value) via the existing `show config`/`show settings` command. Any future voice configuration should follow this exact same pattern rather than inventing a second configuration mechanism:

- A `VOICE_ENABLED` (or similarly named) boolean flag, defaulting to `False` (opt-in, per Section 3.10).
- A selected voice/style identifier, once Nathan has chosen one (Section 3.1/3.2).
- A selected TTS provider identifier (local/OS/cloud/premium — Section 3.3/3.4/3.6 above).
- A selected STT provider identifier, once input is being built (Section 3.5/3.6).
- A "speak mode" flag/enum (Section 3.9 — e.g. all responses vs. none).
- A push-to-talk trigger setting (e.g. which key/typed command starts a recording), once STT is being built.
- Rate/speed/volume settings for TTS output, if the chosen provider supports tuning them.
- Whether `ConfigTool`/`show config` should be extended to report voice settings' status the same redacted way it reports the AI provider today — this plan's recommendation is yes, for consistency, but no code should change until an implementation batch is approved.

None of the above should be implemented in this planning phase.
...
- **Batch 2 — CLI opt-in voice-output wiring + config.** Wire `VoiceOutputService` into `ui/cli.py` so a response can actually be spoken through it (still using the fake provider, or a real one only if Nathan has chosen one by then), behind an opt-in setting/command. Add the `VOICE_ENABLED`/speak-mode configuration (Section 9), following `config/settings.py`'s existing pattern.
- **Batch 3 — push-to-talk STT input, fake provider first.** Add the STT provider abstraction and a fake/test provider; wire a push-to-talk trigger (a hotkey or typed command) that records a bounded window, transcribes via the fake provider, and hands the result to `handle_request()` exactly like typed input. A real STT provider is only added once Nathan has explicitly chosen one from Section 7's research.
- **Batch 4 (or later) — closure, adversarial safety tests, and documentation.** The tests described in Section 10 (approval-bypass-proof tests, TTS/STT-failure-isolation tests, disabled-by-default proof), plus README/user_guide updates and a completion report — mirroring how every prior phase in this project has closed.
