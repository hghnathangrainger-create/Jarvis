...
**Status:** **CLOSED, as a voice foundation phase.** Batches 1–6 are all complete: TTS foundation (1), opt-in CLI voice-output wiring + config (2), STT foundation (3), fake-provider-only CLI voice input routing + adversarial approval-bypass proof (4), push-to-talk trigger design review (5), and closure-scope reconciliation (6, this batch) — see Section 12 for exactly what was delivered in each, and Section 14 for the Batch 5 design review. See `docs/phase_41_completion_report.md` for the full closure report. **Real push-to-talk/microphone capture was never selected for implementation in Phase 41 and is explicitly not required for this phase to close** — Nathan decided to defer it, along with real STT/TTS provider selection, wake-word/always-listening, dashboard voice controls, phone integration, and a Core service, to their own future, separately-planned phase(s) or review(s), exactly like every other deferred capability in this project's history. Phase 41 delivered a complete, safe, fake-provider-only voice foundation — nothing about closing it now implies real audio capability exists or is imminent.
...
**Update (Batch 6, closure): the path actually taken was Planning → Batch 1 (TTS foundation) → Batch 2 (CLI output wiring + config) → Batch 3 (STT foundation) → Batch 4 (fake-provider CLI input routing + adversarial proof) → Batch 5 (push-to-talk trigger design review) → Batch 6 (this closure batch).** Real push-to-talk/STT input never reached implementation within Phase 41 — Nathan explicitly decided not to proceed into it here, since real microphone capture is a new privacy and dependency boundary this project has never crossed, and deferred it to its own future, separately-planned phase, to be opened only once he explicitly chooses a trigger mechanism and capture boundary from Section 14. This is a narrower outcome than this section originally sketched ("Batch 3: push-to-talk STT input") — consistent with how Batches 1, 3, and 5 were each already narrower than first sketched, for the same reason each time: build the safe foundation/design first, not a guessed-at shape.
...
- **Batch 6 — COMPLETE. Closure-scope reconciliation and final closure.** Nathan explicitly decided not to proceed into real push-to-talk implementation inside Phase 41: real microphone capture is a new privacy and dependency boundary this project has never crossed, and remains riskier than what this phase was scoped to deliver. This batch reconciled the plan so it no longer implies real push-to-talk must be built before Phase 41 can close, corrected a small number of now-stale docstring/comment references to "a future batch's real push-to-talk" (updated to "a future, separately-planned phase's" — `ui/cli.py`, `tests/unit/test_voice_input.py`), and produced `docs/phase_41_completion_report.md`. No production behavior changed. Phase 41 is now closed as a voice **foundation** phase: planning, fake/mock TTS and STT foundations, disabled-by-default output/input settings, opt-in CLI wiring for both, a proven-safe shared request/security/approval path for voice-originated text, and a push-to-talk trigger design review — not as a real-audio phase.
- **Real push-to-talk / audio-capture implementation: explicitly deferred to a future, separately-planned phase — not a Phase 41 batch.** Once Nathan picks a trigger mechanism and capture boundary from Section 14, that future phase would decide and implement the actual recording trigger and the audio-capture shape `SpeechToTextProvider.transcribe()` deliberately left unspecified through Batches 3–5, and call `_handle_voice_input_once()` (or an equivalent) from it. It will need a `pyproject.toml` edit and Nathan's explicit approval of a specific new dependency — both standing non-goals of Phase 41 itself. A real STT provider is only added once Nathan has explicitly chosen one from Section 7's research; a real TTS provider only once Nathan has explicitly chosen one from Section 6's research.
- **Always-listening/wake-word, dashboard voice controls, phone integration, and a Core service: all explicitly deferred**, none scheduled as any Phase 41 batch, none implied by this phase's closure. Each would require its own separate future architectural review, on its own merits, entirely independent of whether or when the real push-to-talk phase above happens.
...
This section exists so a future, separately-planned real-capture phase starts from a concrete, compared set of options, instead of guessing. **Nothing in this section is implemented. No option below has been chosen yet — that choice belongs to Nathan, before that future phase begins.**
...
**Whichever is chosen, a hard maximum capture duration must exist regardless** — this is a non-negotiable safety property for that future phase, not an open question: unbounded capture is indistinguishable from always-listening, which remains explicitly deferred.

### 14.3 Adversarial tests a future real-capture phase will need

Recorded now so that future phase does not have to rediscover them:
...
- A dependency-boundary canary test confirming exactly one new, explicitly-approved dependency was added to `pyproject.toml` for that future phase — not more than what Nathan approved.

### 14.4 What Batch 5 deliberately did not decide

No trigger mechanism, no capture boundary, and no audio-capture library were chosen in this review — all three remain Nathan's decision, to be made explicitly before that future phase begins, exactly like every other implementation choice in this project.
