...
**Status:** planning complete; Nathan has recorded temporary selected defaults for several of the decisions below (see the section immediately following this header) — these are explicitly changeable, before or during implementation, and do not close off any option. **Batch 1 (TTS provider abstraction, fake provider, voice output service — foundation only, no CLI wiring, no microphone, no real TTS dependency) is complete. Batch 2 (opt-in, disabled-by-default CLI wiring + voice config settings, still no real TTS dependency, still no microphone) is complete. Batch 3 (STT provider abstraction, fake provider, voice input service — foundation only, still no microphone, still no push-to-talk, transcribed text still not routed/executed anywhere) is now also complete** — see Section 12 for exactly what was delivered in each and what remains for later batches. Phase 41 has not closed yet - no completion report exists, and push-to-talk wiring/execution is still future work.
...
- **Batch 4 (or later) — push-to-talk STT input, fake provider first.** Decide and implement the audio-capture trigger/shape (a hotkey or typed command that records a bounded window), wire `VoiceInputService` into `ui/cli.py` behind it, and hand the resulting transcription to `handle_request()` exactly like typed input - this is also where the trust/origin boundary above must be explicitly, adversarially tested (approval-bypass-proof tests, bad-transcription-cannot-hidden-write tests). A real STT provider is only added once Nathan has explicitly chosen one from Section 7's research.
- **Batch 5 (or later) — closure, remaining adversarial safety tests, and documentation.** Any tests from Section 10 not already covered by Batch 4, plus README/user_guide updates and a completion report — mirroring how every prior phase in this project has closed.
