...
- **Current status:** Deferred. Phase 47 confirmed, by direct live testing, that console logging does not actually work for INFO-level events today (independent of `LOG_LEVEL` entirely) and compared two remediation paths — Option A (wire a real handler using `settings.log_level`) and Option B (leave behavior as-is, correct the docstring). Phase 48 implemented Option B. Option A remains fully available as its own future phase.
- **Why it remains deferred:** It's a genuine product decision (does Nathan want live, noisier console output restored, or is the audit log's durable record already sufficient), not an engineering default to assume either way.
- **Fuller source:** `docs/phase_47_logging_console_visibility_plan.md` (Section 4 names six implementation risks — handler idempotency, the test suite's frequent `build_orchestrator()` calls, avoiding duplicate handlers, entry-point scope, `dashboard.py` exclusion, and console-volume-as-a-UX-decision — that should be respected if Option A is ever undertaken).
