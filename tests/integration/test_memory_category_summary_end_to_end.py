...
def test_category_selection_order_survives_context_budget_pressure_end_to_end() -> (
    None
):
    ...
    in test_memory_recent_summary_end_to_end.py) has no Phase 12 analogue
    until now. Ten real, large category-matching records exceed Phase
    10's own max_total_chars=20,000 default; because
    select_memory_ids_by_category() hands its own created_at DESC, id DESC
    order to ingest_memories_for_ai() unchanged (never reordered), Phase
    10's existing streaming omission drops the *oldest* of the ten
    matching records first - the newest survive, exactly as the recency
    workflow already proves. No production code is changed by this
    regression proof."""
    memory = _memory_manager()
    ids = [
        _save(
            memory,
            f"budget-check-category-{i:02d} " + ("x" * 2480),
            category="project",
        )
        for i in range(10)
    ]
