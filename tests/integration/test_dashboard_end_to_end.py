...
def _build_stack(db_path: Path):
    ...
    return engine, memory, approvals, workflows, inbox, schedules, read_model
...
def test_full_pipeline_real_data_to_rendered_ui_state(
    ...
    _engine, memory, approvals, workflows, _inbox, _schedules, read_model = _build_stack(db_path)
...
def test_refresh_sees_a_committed_memory_write_from_another_session(
    ...
    _engine, memory, _approvals, _workflows, _inbox, _schedules, read_model = _build_stack(db_path)
...
def test_refresh_sees_a_committed_approval_history_write(
    ...
    _engine, _memory, approvals, _workflows, _inbox, _schedules, read_model = _build_stack(db_path)
...
def test_refresh_sees_a_committed_workflow_transition(
    ...
    _engine, _memory, _approvals, workflows, _inbox, _schedules, read_model = _build_stack(db_path)
...
def test_write_via_one_connection_is_read_via_a_second_independent_connection(
    ...
    writer_engine, writer_memory, _, _, _, _, _ = _build_stack(db_path)
    writer_memory.save("from the writer connection")
    writer_engine.dispose()

    reader_engine, reader_memory, _, _, _, _, reader_read_model = _build_stack(db_path)
    try:
...
def test_repeated_open_read_close_cycles_do_not_accumulate_errors(
    ...
    setup_engine, setup_memory, _, _, _, _, _ = _build_stack(db_path)
...
def test_sequential_write_then_read_interleaving_succeeds_repeatedly(
    ...
    writer_engine, writer_memory, writer_approvals, writer_workflows, _, _, _ = (
    ...
    for i in range(10):
        ...
        reader_engine, _, _, _, _, _, reader_read_model = _build_stack(db_path)
        try:
